...
async function initializeDataIfNeeded() {
  try {
    ...
    // Проверяем существование файлов и созданием их при необходимости
    const checkAndInitializeFile = async (filePath, defaultDataPath) => {
      try {
        await fs.access(filePath);
      } catch {
        // Файл не существует, создаем его с примерными данными
        const defaultData = require(defaultDataPath);
        await saveData(filePath, defaultData);
      }
    };
    
    await checkAndInitializeFile(EXCHANGES_FILE, './src/data/mockExchanges');
    await checkAndInitializeFile(PLANTS_FILE, './src/data/mockData').then(() => {
      // Для растений нужно извлечь только массив растений из импортированного модуля
      return loadData(PLANTS_FILE).then(data => {
        if (!Array.isArray(data)) {
          const mockData = require('./src/data/mockData');
          return saveData(PLANTS_FILE, mockData.mockPlants);
        }
      });
    });
    await checkAndInitializeFile(USERS_FILE, './src/data/mockData').then(() => {
      // Для пользователей нужно извлечь только массив пользователей
      return loadData(USERS_FILE).then(data => {
        if (!Array.isArray(data)) {
          const mockData = require('./src/data/mockData');
          return saveData(USERS_FILE, mockData.mockUsers);
        }
      });
    });
